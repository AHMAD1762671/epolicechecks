
<div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="row dashboard-sevices">

            <div class="col-lg-2 col-md-4 col-sm-6">
                <a href="<?php echo base_url('assets').'/namebased_report.xlsx'; ?>">
                    <div class="card bg-gradient-info overflow-hidden text-white" style="background-image: linear-gradient(to bottom, #4a85fb 0, #163eaf 100%); box-shadow: 0 2px 10px rgb(22 62 175 / 30%);}">
                        <div class="card-body pb-0">
                            <p style="font-size: 15px; text-align: center;" class=" mb-1 "><b> Download Excel  </b></p>
                            <p style="font-size: 20px; text-align: center;" class=" mb-1 "><b></b></p>
                        </div>
                        <br>
                    </div>
                </a>
            </div>

            <div class="col-lg-2 col-md-4 col-sm-6">
                <div class="card bg-gradient-info overflow-hidden text-white" style="background-image: linear-gradient(to bottom, #62db17 0, #487a1b 100%); box-shadow: 0 2px 10px rgb(215 46 4 / 30%);}">
                    <div class="card-body pb-0">
                        <p style="font-size: 15px; text-align: center;" class=" mb-1 "><b>  </b></p>
                        <p style="font-size: 20px; text-align: center;" class=" mb-1 "><b></b></p>
                    </div>
                    <br>
                </div>
            </div>

            <div class="col-lg-2 col-md-4 col-sm-6">
                <div class="card bg-gradient-info overflow-hidden text-white" style="background-image: linear-gradient(to bottom, #fc7a5a 0, #d72e04 100%); box-shadow: 0 2px 10px rgb(215 46 4 / 30%);}">
                    <div class="card-body pb-0">
                        <p style="font-size: 15px; text-align: center;" class=" mb-1 "><b>  </b></p>
                        <p style="font-size: 20px; text-align: center;" class=" mb-1 "><b></b></p>
                    </div>
                    <br>
                </div>
            </div>




            <!--            <div class="col-lg-2 col-md-4 col-sm-6">-->
            <!--                <a href="--><?php //echo base_url('account').'/custom_invoices'; ?><!--">-->
            <!--                    <div class="card bg-gradient-info overflow-hidden text-white" style="background-image: linear-gradient(to bottom, #fc7a5a 0, #d72e04 100%); box-shadow: 0 2px 10px rgb(215 46 4 / 30%);}">-->
            <!--                        <div class="card-body pb-0">-->
            <!--                            <p style="font-size: 15px; text-align: center;" class=" mb-1 "><b> Generate Custom </b></p>-->
            <!--                            <p style="font-size: 15px; text-align: center;" class=" mb-1 "><b>Invoice</b></p>-->
            <!--                        </div>-->
            <!--                        <br>-->
            <!--                    </div>-->
            <!--                </a>-->
            <!--            </div>-->


            <div class="col-lg-1 col-md-4 col-sm-6"> </div>

        </div>
    </div>
</div>

<br/>







<div class="row">
    <div class="col-md-12 col-lg-12">
        <div class="table-responsive">
            <table class="table table-hover">
                <thead>
                <tr>
                    <th class="align-middle" scope="col">Client Id</th>
                    <!--                    <th class="align-middle" scope="col">Service</th>-->
                    <th class="align-middle" scope="col">Application</th>
                    <th class="align-middle" scope="col">Reffered By</th>
                    <th class="align-middle" scope="col">Payment Type</th>
                    <th class="align-middle" scope="col">Amount</th>
                    <th class="align-middle" scope="col">Action</th>
                </tr>
                </thead>
                <tbody>



                <tr>
                    <th class="align-middle">2002</th>
                    <th class="align-middle">Digital Fingerprinting</th>
                    <th class="align-middle">agent@gmail.com</th>
                    <th class="align-middle">Cash</th>
                    <th class="align-middle">35</th>
                    <th class="align-middle">
                        <!--                            <a href="--><?php //echo base_url('account').'/download_invoice/'.$value->invoice_id; ?><!--">-->
                        <!--                                <button class="btn btn-primary" title="download invoice">-->
                        <!--                                    <span class="fa fa-download"></span>-->
                        <!--                                    <input type="hidden" value="--><?php // ?><!--" name="id">-->
                        <!--                                    <input type="hidden" value="on" name="status">-->
                        <!--                                </button>-->
                        <!--                            </a>-->
                        <a href="#">
                            <button class="btn btn-success" title="Send to User">
                                <span class="fa fa-paper-plane"></span>
                                <input type="hidden" value="<?php  ?>" name="id">
                                <input type="hidden" value="on" name="status">
                            </button>
                        </a>
                    </th>
                </tr>
                <tr>
                    <th class="align-middle">2003</th>
                    <th class="align-middle">Namebased</th>
                    <th class="align-middle">agent@gmail.com</th>
                    <th class="align-middle">Cash</th>
                    <th class="align-middle">35</th>
                    <th class="align-middle">
                        <!--                            <a href="--><?php //echo base_url('account').'/download_invoice/'.$value->invoice_id; ?><!--">-->
                        <!--                                <button class="btn btn-primary" title="download invoice">-->
                        <!--                                    <span class="fa fa-download"></span>-->
                        <!--                                    <input type="hidden" value="--><?php // ?><!--" name="id">-->
                        <!--                                    <input type="hidden" value="on" name="status">-->
                        <!--                                </button>-->
                        <!--                            </a>-->
                        <a href="#">
                            <button class="btn btn-success" title="Send to User">
                                <span class="fa fa-paper-plane"></span>
                                <input type="hidden" value="<?php  ?>" name="id">
                                <input type="hidden" value="on" name="status">
                            </button>
                        </a>
                    </th>
                </tr>

                <tr>
                    <th class="align-middle">2004</th>
                    <th class="align-middle">Digital Fingerprinting</th>
                    <th class="align-middle">agent@gmail.com</th>
                    <th class="align-middle">Cash</th>
                    <th class="align-middle">35</th>
                    <th class="align-middle">
                        <!--                            <a href="--><?php //echo base_url('account').'/download_invoice/'.$value->invoice_id; ?><!--">-->
                        <!--                                <button class="btn btn-primary" title="download invoice">-->
                        <!--                                    <span class="fa fa-download"></span>-->
                        <!--                                    <input type="hidden" value="--><?php // ?><!--" name="id">-->
                        <!--                                    <input type="hidden" value="on" name="status">-->
                        <!--                                </button>-->
                        <!--                            </a>-->
                        <a href="#">
                            <button class="btn btn-success" title="Send to User">
                                <span class="fa fa-paper-plane"></span>
                                <input type="hidden" value="<?php  ?>" name="id">
                                <input type="hidden" value="on" name="status">
                            </button>
                        </a>
                    </th>
                </tr>

                <tr>
                    <th class="align-middle">2005</th>
                    <th class="align-middle">Namebased</th>
                    <th class="align-middle">agent@gmail.com</th>
                    <th class="align-middle">Cash</th>
                    <th class="align-middle">35</th>
                    <th class="align-middle">
                        <!--                            <a href="--><?php //echo base_url('account').'/download_invoice/'.$value->invoice_id; ?><!--">-->
                        <!--                                <button class="btn btn-primary" title="download invoice">-->
                        <!--                                    <span class="fa fa-download"></span>-->
                        <!--                                    <input type="hidden" value="--><?php // ?><!--" name="id">-->
                        <!--                                    <input type="hidden" value="on" name="status">-->
                        <!--                                </button>-->
                        <!--                            </a>-->
                        <a href="#">
                            <button class="btn btn-success" title="Send to User">
                                <span class="fa fa-paper-plane"></span>
                                <input type="hidden" value="<?php  ?>" name="id">
                                <input type="hidden" value="on" name="status">
                            </button>
                        </a>
                    </th>
                </tr>
                <tr>
                    <th class="align-middle">2006</th>
                    <th class="align-middle">Namebased</th>
                    <th class="align-middle">agent@gmail.com</th>
                    <th class="align-middle">Cash</th>
                    <th class="align-middle">35</th>
                    <th class="align-middle">
<!--                        <a href="--><?php //echo base_url('account').'/download_invoice/'.$value->invoice_id; ?><!--">-->
<!--                            <button class="btn btn-primary" title="download invoice">-->
<!--                                <span class="fa fa-download"></span>-->
<!--                                <input type="hidden" value="--><?php // ?><!--" name="id">-->
<!--                                <input type="hidden" value="on" name="status">-->
<!--                            </button>-->
<!--                        </a>-->
                        <a href="#">
                            <button class="btn btn-success" title="Send to User">
                                <span class="fa fa-paper-plane"></span>
                                <input type="hidden" value="<?php  ?>" name="id">
                                <input type="hidden" value="on" name="status">
                            </button>
                        </a>
                    </th>
                </tr>

                <tr>
                    <th class="align-middle">2007</th>
                    <th class="align-middle">Digital Fingerprinting</th>
                    <th class="align-middle">agent@gmail.com</th>
                    <th class="align-middle">Cash</th>
                    <th class="align-middle">35</th>
                    <th class="align-middle">
                        <!--                            <a href="--><?php //echo base_url('account').'/download_invoice/'.$value->invoice_id; ?><!--">-->
                        <!--                                <button class="btn btn-primary" title="download invoice">-->
                        <!--                                    <span class="fa fa-download"></span>-->
                        <!--                                    <input type="hidden" value="--><?php // ?><!--" name="id">-->
                        <!--                                    <input type="hidden" value="on" name="status">-->
                        <!--                                </button>-->
                        <!--                            </a>-->
                        <a href="#">
                            <button class="btn btn-success" title="Send to User">
                                <span class="fa fa-paper-plane"></span>
                                <input type="hidden" value="<?php  ?>" name="id">
                                <input type="hidden" value="on" name="status">
                            </button>
                        </a>
                    </th>
                </tr>


                </tbody>

            </table>
        </div>
        <!--        --><?php //echo $this->paginator->get_links(); ?>
    </div>
</div>
